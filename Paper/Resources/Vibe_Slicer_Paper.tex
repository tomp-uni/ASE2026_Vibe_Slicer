...
\begin{center}
  ...
  {\normalsize Department of Computer Sciences, University of Salzburg, Austria}\\
...
The benchmarks showed that theoretical accuracy is identical to Cura under matched parameters;
practical accuracy is within $\pm$0.3\,mm for outer dimensions and
within $\pm$0.2\,mm for holes.
...
Vibe Slicer was developed over the course of the Winter 2026
...
offset plus the 0.200\,mm half-line-width correction
...
Solid top layers & 3 \\
Infill density & 25\,\% \\
Cooling fan & On from layer 1 at 100 \% \\
Line width & 0.4\,mm \\
...
with a hardware-level bed inaccuracy on the test printer.
...
on average. The reason for the larger deviations with Vibe Slicer was found to mainly be overall roughness of the Holes, since Vibe Slicer uses a significantly lower resolution and no path-smoothing compared to Cura for round contours. Overall, all measured dimensions fall within the $\pm$0.3\,mm tolerance typical of consumer FDM hardware.

Table~\ref{tab:summary} provides a consolidated comparison,
with the Cura-Optimised column set apart by a vertical rule.

\begin{table}[H]
\centering
\caption{Summary: mean deviation from nominal (mm) and mean
print time. Deviation is the mean signed deviation across all
measured axes of each feature (only $X$ and $Y$ diameters for holes). The Cura-Optimised column
(right of the rule) is the reference baseline defined in
...
This is likely attributable to Cura's arc-aware hole
...
The author thanks Prof. Christoph Kirsch, who was the lecturer in the Advanced Systems Engineering class at the University of Salzburg for their feedback
