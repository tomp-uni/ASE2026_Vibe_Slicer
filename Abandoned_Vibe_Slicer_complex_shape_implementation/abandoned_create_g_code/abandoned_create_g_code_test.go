...
func TestOffsetPolygonRejectsRunawaySecondWallOnComplexContour(t *testing.T) {
	...
	for _, p := range second {
		if math.Abs(p.X) > 300 || math.Abs(p.Y) > 300 {
			t.Fatalf("expected runaway second offset to stay bounded or be rejected, got point %+v", p)
		}
	}
}

func TestOffsetPolygonUsesBoundedJoinOnSharpCorner(t *testing.T) {
	points := []Point2D{
		{X: 0, Y: 0},
		{X: 0, Y: 20},
		{X: 20, Y: 20},
	...
	offset := offsetPolygon(points, 0.4)
	if offset == nil {
		t.Fatal("expected sharp corner offset to stay bounded")
	}
	for _, p := range offset {
		if math.Abs(p.X) > 22 || math.Abs(p.Y) > 22 {
			t.Fatalf("expected bounded offset points, got %+v", p)
		}
	}
	if !containsPointApprox(offset, Point2D{X: 0.4, Y: 0.4}, 0.8) {
		t.Fatalf("expected outer corner offset to remain near the original corner, got %+v", offset)
	}
}

func TestOffsetPolygonRejectsNearlyParallelSpike(t *testing.T) {
	points := []Point2D{
		{X: 0, Y: 0},
		{X: 0, Y: 20},
		{X: 20, Y: 20},
	...
	for _, p := range offset {
		if math.Abs(p.X) > 22 || math.Abs(p.Y) > 22 {
			t.Fatalf("expected no runaway spike from nearly parallel edges, got %+v", p)
		}
	}
}

func TestOffsetPolygonKeepsSquareRegressionStable(t *testing.T) {
	square := []Point2D{{X: 0, Y: 0}, {X: 0, Y: 10}, {X: 10, Y: 10}, {X: 10, Y: 0}}

	inner := insetPolygon(square, 0.2)
	if inner == nil {
		t.Fatal("expected square inset to succeed")
	}
	want := []Point2D{{X: 0.2, Y: 0.2}, {X: 0.2, Y: 9.8}, {X: 9.8, Y: 9.8}, {X: 9.8, Y: 0.2}}
	for i, p := range want {
		if !pointsApproxEqual(inner[i], p, 1e-3) {
			t.Fatalf("expected inset corner %d to be %+v, got %+v", i, p, inner[i])
		}
	}
}

func containsPointApprox(points []Point2D, want Point2D, tol float64) bool {
	for _, p := range points {
		if pointsApproxEqual(p, want, tol) {
			return true
		}
	}
	return false
...
func pointsApproxEqual(a, b Point2D, tol float64) bool {
...
func TestRealModelInfillStaysInsideConcaveContour(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join("..", "xyzcube.json"))
	if err != nil {
		t.Fatalf("read calibration slices: %v", err)
	}

	var slices SliceOutput
	if err := json.Unmarshal(raw, &slices); err != nil {
		t.Fatalf("decode calibration slices: %v", err)
	}

	var layer LayerResult
	for _, candidate := range slices.Layers {
		if math.Abs(candidate.Z-10.0) <= 1e-6 && len(candidate.Contours) > 0 {
			layer = candidate
			break
		}
	}
	if len(layer.Contours) == 0 {
		t.Fatal("expected to find the concave middle calibration layer")
	}

	input := SliceOutput{
		Input:       slices.Input,
		LayerHeight: slices.LayerHeight,
	...
	gcode := buildGCode(input, cfg)
	start := strings.Index(gcode, "; INFILL LAYER 0 DENSITY=100 ANGLE=45")
	if start == -1 {
		t.Fatalf("expected infill gcode for the real calibration layer")
	}
	section := gcode[start:]
	for _, line := range strings.Split(section, "\n") {
		if !strings.HasPrefix(line, "G1 X") {
			continue
		}
		var x, y float64
		if _, err := fmt.Sscanf(line, "G1 X%f Y%f", &x, &y); err != nil {
			t.Fatalf("parse infill move %q: %v", line, err)
		}
		p := Point2D{X: x, Y: y}
		if !pointInPolygon(layer.Contours[0].Points, p) {
			t.Fatalf("expected infill move to stay inside the concave contour, got %+v from %q", p, line)
		}
	}
}
